...
void Layer::init_weight_bias(unsigned int seed) 
{
    ...
    weights_->to(DeviceType::cuda, device_num_);
    biases_->to(DeviceType::cuda, device_num_);
    weights_m_->to(DeviceType::cuda, device_num_);
    weights_v_->to(DeviceType::cuda, device_num_);
    biases_m_->to(DeviceType::cuda, device_num_);
    biases_v_->to(DeviceType::cuda, device_num_);
    ...
    if (weights_ != nullptr && grad_weights_ != nullptr) {
        ...
        checkCublasErrors(
                cublasSaxpy(cuda_->cublas(),
                            weights_->len(),
                            &eps,
                            grad_weights_->cuda(device_num_), 1,
                            weights_->cuda(device_num_), 1));
    ...
    if (biases_ != nullptr && grad_biases_ != nullptr) {
        ...
        checkCublasErrors(
                cublasSaxpy(cuda_->cublas(),
                            biases_->len(),
                            &eps,
                            grad_biases_->cuda(device_num_), 1,
                            biases_->cuda(device_num_), 1));
...
//                              (weights_->len(), step, weights_m_->cuda(device_num_),
//                              weights_v_->cuda(device_num_), weights_->cuda(device_num_),
//                              grad_weights_->cuda(device_num_), beta1, beta2, eps_hat, learning_rate);
...
//         adam_update <<<config.block_count,config.thread_per_block>>>
//                              (biases_->len(), step, biases_m_->cuda(device_num_),
//                              biases_v_->cuda(device_num_), biases_->cuda(device_num_),
//                              grad_biases_->cuda(device_num_), beta1, beta2, eps_hat,
//                              learning_rate);
...
//         momentum_update <<< config.block_count, config.thread_per_block >>> (weights_->len(), weights_m_->cuda(device_num_), weights_->cuda(device_num_), grad_weights_->cuda(device_num_), learning_rate, momentum);
...
//         momentum_update <<< config.block_count, config.thread_per_block >>> (biases_->len(), biases_m_->cuda(device_num_), biases_->cuda(device_num_), grad_biases_->cuda(device_num_), learning_rate, momentum);
...
int Layer::load_parameter() {
    ...
    if (weights_->file_read(filename_weights.str(), device_num_))
        return -1;

    filename_biases << name_ << ".bias.bin";
    if (biases_->file_read(filename_biases.str(), device_num_))
        return -2;
...
int Layer::save_parameter() {
    ...
    if (weights_) {
        filename_weights << name_ << ".bin";
        if (weights_->file_write(filename_weights.str(), device_num_))
            return -1;
    }
    ...
    if (biases_) {
        filename_biases << name_ << ".bias.bin";
        if (biases_->file_write(filename_biases.str(), device_num_))
            return -2;
    }
...
Blob<float> *Layer::sum_gradients(Blob<float> *grad) {
    ...
    if (copy_output_to_ != nullptr) {
        ...
        add<<<config.block_count,config.thread_per_block>>>(grad->cuda(device_num_), grad2->cuda(device_num_), grad->cuda(device_num_), grad->len());
        cudaDeviceSynchronize();
    }
    return grad;
}
...
Dense::Dense(Layer *input_from, std::string name, int device_num, int output_size) {
...
Blob<float> *Dense::forward(Blob<float> *input) {
    ...
    if (input_ == nullptr || batch_size_ != input->n()) {
        ...
        if (output_ == nullptr)
            ...
            output_->reset(device_num_, batch_size_, output_size_);
        ...
        checkCudaErrors(cudaSetDevice(device_num_));
        checkCudaErrors(cudaMallocManaged((void **) &d_one_vec, sizeof(float) * batch_size_));
    ...
    checkCublasErrors(cublasSgemm(cuda_->cublas(),
                        ...
                        &cuda_->one,
                        weights_->cuda(device_num_), input_size_,
                        input_->cuda(device_num_), input_size_,
                        &cuda_->zero,
                        output_->cuda(device_num_), output_size_));
    ...
    checkCublasErrors(cublasSgemm(cuda_->cublas(),
                                  ...
                                  &cuda_->one,
                                  biases_->cuda(device_num_), output_size_,
                                  d_one_vec, 1,
                                  &cuda_->one,
                                  output_->cuda(device_num_), output_size_));
...
Blob<float> *Dense::backward(Blob<float> *grad_output) {
    ...
    if (grad_input_ == nullptr || batch_size_ != grad_output->n()) {
        ...
        if (grad_input_ == nullptr)
            ...
            grad_input_->reset(device_num_, input_->shape());
    }
    ...
    cublasSgemv(cuda_->cublas(),
                ...
                &cuda_->one,
                grad_output_->cuda(device_num_), output_size_,
                d_one_vec, 1,
                &cuda_->zero,
                grad_biases_->cuda(device_num_), 1);
    ...
    cublasSgemm(cuda_->cublas(),
                ...
                grad_output_->cuda(device_num_), output_size_,
                &cuda_->zero,
                grad_weights_->cuda(device_num_), input_size_);
    ...
    if (!gradient_stop_)
        cublasSgemm(cuda_->cublas(),
                    ...
                    grad_output_->cuda(device_num_), output_size_,
                    &cuda_->zero,
                    grad_input_->cuda(device_num_), input_size_);
...
Activation::Activation(Layer *input_from, std::string name, int device_num, cudnnActivationMode_t mode, float coef) {
    set_layer_relationship(input_from);
...
Blob<float> *Activation::forward(Blob<float> *input) {
    ...
    if (input_ == nullptr || batch_size_ != input->n()) {
        ...
        if (output_ == nullptr)
            ...
            output_->reset(device_num_, input->shape());
    ...
    cudnnActivationForward(cuda_->cudnn(),
                           ...
                           &cuda_->one,
                           input_desc_,
                           input->cuda(device_num_),
                           &cuda_->zero,
                           output_desc_,
                           output_->cuda(device_num_));
...
Blob<float> *Activation::backward(Blob<float> *grad_output) {
    ...
    cudnnActivationBackward(cuda_->cudnn(),
                            act_desc_,
                            &cuda_->one,
                            output_desc_, output_->cuda(device_num_),
                            output_desc_, grad_output->cuda(device_num_),
                            input_desc_, input_->cuda(device_num_),
                            &cuda_->zero,
                            input_desc_, grad_input_->cuda(device_num_));
 ...
 * Softmax definition                                           *
 ****************************************************************/
Softmax::Softmax(Layer *input_from,std::string name, int device_num) {
    set_layer_relationship(input_from);
...
Blob<float> *Softmax::forward(Blob<float> *input) {
    ...
    checkCudnnErrors(
            cudnnSoftmaxForward(cuda_->cudnn(), CUDNN_SOFTMAX_ACCURATE, CUDNN_SOFTMAX_MODE_CHANNEL,
                                &cuda_->one, input_desc_, input->cuda(device_num_),
                                &cuda_->zero, output_desc_, output_->cuda(device_num_)));
...
Blob<float> *Softmax::backward(Blob<float> *target) {
    ...
    checkCudaErrors(cudaMemcpyAsync(grad_input_->cuda(device_num_),
                                    output_->cuda(device_num_), output_->buf_size(),
                                    cudaMemcpyDeviceToDevice));
    // set grad_input_ = predict - target
    checkCublasErrors(
            cublasSaxpy(cuda_->cublas(), target->len(),
                        &cuda_->minus_one, target->cuda(device_num_), 1,
                        grad_input_->cuda(device_num_), 1));

    // normalize the grad_output by the batch size
    float scale = 1.f / static_cast<float>(target->n());
    checkCublasErrors(cublasSscal(cuda_->cublas(), target->len(), &scale, grad_input_->cuda(device_num_), 1));
...
int Softmax::get_accuracy(Blob<float> *target) {
    ...
    h_output = output_->to(host, device_num_);
    h_target = target->to(host, device_num_);
    cudaDeviceSynchronize();
...
Conv2D::Conv2D(Layer *input_from,
    ...
    // std::cout<<"Conv2D:: constructor():: name is " << name << std::endl;
    // printf("\tchannels %d, kernel_size %d, stride %d, padding %d, dilation %d, device_num %d\n",out_channels, kernel_size,stride,padding,dilation,device_num);
    name_ = name;
    device_num_ = device_num;
    ...
    checkCudnnErrors(cudnnSetConvolution2dDescriptor(conv_desc_, padding_, padding_, stride_, stride_, dilation_, dilation_, CUDNN_CROSS_CORRELATION, CUDNN_DATA_FLOAT));
}
...
void Conv2D::set_workspace() {
    ...
    if (workspace_size > 0) {
		...
		checkCudaErrors(cudaSetDevice(device_num_));
        checkCudaErrors(cudaMallocManaged((void **) &d_workspace, workspace_size));
    }
}
...
Blob<float> *Conv2D::forward(Blob<float> *input) {
    ...
    if (input_ == nullptr || batch_size_ != input->n()) {
        ...
        if (output_ == nullptr)
            ...
            output_->reset(device_num_, output_size_);
    ...
    checkCudnnErrors(cudnnConvolutionForward(cuda_->cudnn(),
                                             &cuda_->one, input_desc_, input_->cuda(device_num_),
                                             filter_desc_, weights_->cuda(device_num_), conv_desc_, conv_fwd_algo_, d_workspace,
                                             workspace_size,
                                             &cuda_->zero, output_desc_, output_->cuda(device_num_)));

    checkCudnnErrors(cudnnAddTensor(cuda_->cudnn(),
                                    &cuda_->one, bias_desc_, biases_->cuda(device_num_),
                                    &cuda_->one, output_desc_, output_->cuda(device_num_)));
...
Blob<float> *Conv2D::backward(Blob<float> *grad_output) {
    ...
    checkCudnnErrors(
            cudnnConvolutionBackwardBias(cuda_->cudnn(),
                                         ...
                                         output_desc_, grad_output->cuda(device_num_),
                                         &cuda_->zero,
                                         bias_desc_, grad_biases_->cuda(device_num_)));
 ...
 * Add definition                                               *
 ****************************************************************/
Add::Add(Layer *input_from, Layer *input2_from, std::string name, int device_num) {
...
Blob<float> *Add::forward(Blob<float> *input) {
    ...
    // y = x + x2
    add<<<config.block_count, config.thread_per_block>>>(input_->cuda(device_num_), input2_->cuda(device_num_), output_->cuda(device_num_), input_->len());
...
Blob<float> *Add::backward(Blob<float> *grad_output) {
    ...
    // y = x + x2
    // dy/dx = dy/dx2 = 1
    checkCudaErrors(cudaMemcpy(grad_input_->cuda(device_num_),
                               grad_output_->cuda(device_num_), grad_output_->buf_size(),
                               cudaMemcpyDeviceToDevice));
 ...
 ****************************************************************/
/*
Pad::Pad(Layer *input_from, int device_num, std::string name, std::array<int, 8> paddings, int pad_value) {
    set_layer_relationship(input_from);
...
Blob<float> *Pad::forward(Blob<float> *input) {
    ...
    if (input_ == nullptr || batch_size_ != input->n()) {
        ...
        if (output_ == nullptr)
            ...
            output_->reset(device_num_, n, c, h, w);
    }
    // eigen implemented.
     Eigen::GpuStreamDevice stream;
     Eigen::GpuDevice gpu_device(&stream);
     Eigen::TensorMap <Eigen::Tensor<float, 4, Eigen::RowMajor>> gpu_in(input_->cuda(device_num_), input_->n(), input_->c(),
	 	                                      input_->h(), input_->w());
    
     printf("Error here b\n");
     Eigen::TensorMap <Eigen::Tensor<float, 4, Eigen::RowMajor>> gpu_out(output_->cuda(device_num_), output_->n(), output_->c(),
	 	                                      output_->h(), output_->w());
    ...
    //  gpu_out.device(0) = gpu_in.pad(pads, pad_value_);
...
Blob<float> *Pad::backward(Blob<float> *grad_output) {
	...
	int w = grad_output_->w() - paddings_.at(6) - paddings_.at(7);
        if (grad_input_ == nullptr)
            ...
            grad_input_->reset(device_num_, n, c, h, w);
    ...
    if (!gradient_stop_) {
        // eigen implemented.
         Eigen::GpuStreamDevice stream;
         Eigen::GpuDevice gpu_device(&stream);
         Eigen::TensorMap <Eigen::Tensor<float, 4, Eigen::RowMajor>> gpu_in(grad_output_->cuda(device_num_),
	                                                   grad_output_->n(), grad_output_->c(),
                                                           grad_output_->h(), grad_output_->w());
         Eigen::TensorMap <Eigen::Tensor<float, 4, Eigen::RowMajor>> gpu_out(grad_input_->cuda(device_num_),
 ...
 ****************************************************************/
FusedBatchNormalization::FusedBatchNormalization(Layer *input_from, std::string name, int device_num, cudnnBatchNormMode_t mode) {
    set_layer_relationship(input_from);
...
Blob<float> *FusedBatchNormalization::forward(Blob<float> *input) {
    ...
    if (input_ == nullptr || batch_size_ != input->n()) {
		...
		checkCudaErrors(cudaSetDevice(device_num_)); 
        cudaMallocManaged(&resultRunningMean_, sizeof(float) * size_);
        cudaMallocManaged(&resultRunningVariance_, sizeof(float) * size_);

        cudaMallocManaged(&resultSaveMean_, sizeof(float) * size_);
        cudaMallocManaged(&resultSaveInvVariance_, sizeof(float) * size_);
    ...
    checkCudnnErrors(
            cudnnBatchNormalizationForwardTraining(cuda_->cudnn(), mode_, &cuda_->one, &cuda_->zero, input_desc_,
                                                   input_->cuda(device_num_), output_desc_, output_->cuda(device_num_),
                                                   bnScaleBiasMeanVarDesc_, weights_->cuda(device_num_), biases_->cuda(device_num_),
...
Blob<float> *FusedBatchNormalization::backward(Blob<float> *grad_output) {
    ...
    checkCudnnErrors(
            cudnnBatchNormalizationBackward(cuda_->cudnn(), mode_, &cuda_->one, &cuda_->zero, &cuda_->one, &cuda_->zero,
                                            input_desc_, input_->cuda(device_num_), output_desc_, grad_output_->cuda(device_num_), input_desc_,
                                            grad_input_->cuda(device_num_), bnScaleBiasMeanVarDesc_, weights_->cuda(device_num_), grad_weights_->cuda(device_num_),
                                            grad_biases_->cuda(device_num_), CUDNN_BN_MIN_EPSILON, resultSaveMean_, resultSaveInvVariance_));
