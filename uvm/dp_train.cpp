...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < NUM_GPUS; i++) {
        ...
        mainline = model[i].add_layer(new Conv2D(mainline, "conv1", i, 20, 5));
        mainline = model[i].add_layer(new Pooling(mainline, "pool", i, 2, 0, 2, CUDNN_POOLING_MAX));
        mainline = model[i].add_layer(new Conv2D(mainline, "conv2", i, 50, 5));
        mainline = model[i].add_layer(new Pooling(mainline, "pool", i, 2, 0, 2, CUDNN_POOLING_MAX));
